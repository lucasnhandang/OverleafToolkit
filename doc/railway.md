...
3. **Set environment variables** (Web service)
   - `SHARELATEX_MONGO_URL`: Railway Mongo URL, e.g. `mongodb://user:pass@host:port/sharelatex`.
   - `SHARELATEX_REDIS_HOST`, `SHARELATEX_REDIS_PORT`, `SHARELATEX_REDIS_PASSWORD` (if set).
   - `SHARELATEX_SITE_URL`: public Railway URL (e.g. `https://<app>.up.railway.app`). Update after first deploy, then redeploy.
   - `SHARELATEX_APP_NAME`, `SHARELATEX_ADMIN_EMAIL`.
   - SMTP (recommended for invites): `SHARELATEX_EMAIL_FROM_ADDRESS`, `SHARELATEX_SMTP_HOST`, `SHARELATEX_SMTP_PORT` (587), `SHARELATEX_SMTP_USER`, `SHARELATEX_SMTP_PASS`, `SHARELATEX_SMTP_SSL=false`, `SHARELATEX_SMTP_TLS=true`.
   - Optional: `SHARELATEX_MAX_UPLOAD_SIZE=50mb`.
4. **Deploy**
...
6. **Backups**
   - Mongo holds all projects. From a shell with access to Mongo (e.g. local machine using the provided URI):
     ```sh
     mongodump --uri "$SHARELATEX_MONGO_URL" --archive=backup.archive
     ```
